...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model):
        # print(llm_model)
        # t=torch.load(llm_model, map_location=self.device)
        # print(t.keys())
        # for k,v in t.items():
        #     print(k, v.shape)
        # print(t['llm_decoder.weight'].shape)
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid):
        ...
        # print('prompt_text shape', prompt_text.shape)
        # print('llm_prompt_speech_token shape', llm_prompt_speech_token.shape)
        # print('llm_embedding shape',llm_embedding.shape)
        # print('uuid', uuid)
        # print('self.tts_speech_token_dict[uuid] shape', len(self.tts_speech_token_dict[uuid]))
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if source_speech_token.shape[1] == 0:
            ...
            p = threading.Thread(target=self.vc_job, args=(source_speech_token, this_uuid))
        ## 启动多线程，llm推理较慢，为的是llm推理的token一旦达到提前设定的阈值，比如25个token（1s）的时候，立刻开始token2wav的生成
        ## 而后台继续llm的推理
        ## llm后台一直推理，全局本线程如果有足够的token，就开始token2wav
        p.start()
        if stream is True:
            token_offset = 0
            prompt_token_pad = int(np.ceil(flow_prompt_speech_token.shape[1] / self.token_hop_len) * self.token_hop_len - flow_prompt_speech_token.shape[1])
            # print('flow_prompt_speech_token.shape', flow_prompt_speech_token.shape) ##
            # print('self.token_hop_len', self.token_hop_len)
            # print('prompt_token_pad', prompt_token_pad) ## 将prompt speech token拼接到token_hop_len的整数倍，也就是每次吐出来的音频长度，默认是25个token，也就是1s
            while True:
                time.sleep(0.1)
