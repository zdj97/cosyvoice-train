...
def get_args():
    ...
    parser.add_argument('--model_path', default=None, help='save model dir')
...
def main():
    ...
    # print(configs[args.model])
    model = configs[args.model]
    # model = AutoModel(model_dir=args.model_path)
    # print(args.checkpoint)
    start_step, start_epoch = 0, -1
    if args.checkpoint is not None:
        if os.path.exists(args.checkpoint):
            state_dict = torch.load(args.checkpoint, map_location='cpu')
            # print(state_dict['llm_decoder.weight'].shape)
