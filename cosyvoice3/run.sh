...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  cat $data_dir/parquet/data.list > $data_dir/train.data.list
  shuf -n 2 $data_dir/train.data.list > $data_dir/dev.data.list
  ...
  # for model in flow; do
    torchrun --nnodes=1 --nproc_per_node=$num_gpus \
      ...
      --model_dir `pwd`/$data_dir/cosyvoice3/$model/$train_engine \
      --tensorboard_dir `pwd`/$data_dir/tensorboard/cosyvoice3/$model/$train_engine \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  for model in llm flow; do
    decode_checkpoint=`pwd`/$data_dir/cosyvoice3/$model/$train_engine/${model}.pt
    echo "do model average and final checkpoint is $decode_checkpoint"
    python cosyvoice/bin/average_model.py \
      --dst_model $decode_checkpoint \
      --src_path `pwd`/$data_dir/cosyvoice3/$model/$train_engine  \
      --num ${average_num} \
      --val_best
  done
fi
